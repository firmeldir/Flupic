...
enum class FirebaseAuthError(val errorCode: String){
    ...
    fun toStringResource()  =
        when (this) {
            ERROR_INVALID_PHONE_NUMBER -> R.string.invalid_phone_number
            ERROR_TOO_MANY_REQUESTS -> R.string.error_too_many_attempts
            ERROR_QUOTA_EXCEEDED -> R.string.error_quota_exceeded
            ERROR_INVALID_VERIFICATION_CODE -> R.string.incorrect_code_dialog_body
            ERROR_SESSION_EXPIRED -> R.string.error_session_expired

            else -> R.string.encapsulated_error
        }

    companion object{
        fun fromException(e : FirebaseAuthException) =
            try {
        ...
        const val TAG = "TAG FirebaseAuthError"
    }
